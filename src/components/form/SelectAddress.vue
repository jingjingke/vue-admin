  ...
  <el-row class="dialog-address-warp">
    <el-col :span="6">
      <el-select placeholder="选择省" v-model="prov">
        <template v-for="(option, index) of provList">
          <el-option :label="option.address" :value="option.code" :key="'provList' + index"></el-option>
        </template>
      </el-select>
    </el-col>
    <el-col :span="6">
      <el-select placeholder="选择市" v-model="city">
        <template v-for="(option, index) of cityList">
          <el-option :label="option.address" :value="option.code" :key="'cityList' + index"></el-option>
        </template>
      </el-select>
    </el-col>
    <el-col :span="6">
      <el-select placeholder="选择区" v-model="area">
        <template v-for="(option, index) of areaList">
          <el-option :label="option.address" :value="option.code" :key="'areaList' + index"></el-option>
        </template>
      </el-select>
    </el-col>
    <el-col :span="6">
      <template v-if="prov !== ''">
        <el-button type="primary" @click="submitResult" plain>
          {{ btnText || "确 定" }}
        </el-button>
      </template>
    </el-col>
    <el-col :span="24" class="dialog-address-tag">
      <el-tag :key="tag" v-for="(tag, index) in nameList" closable @close="closedTag(index)" size="small">
        {{ tag }}
      </el-tag>
    </el-col>
  </el-row>
...
export default {
  ...
  methods: {
    ...
    submitResult() {
      ...
      if (
        this.$options.propsData.multiple === "" ||
        this.$options.propsData.multiple === true
      ) {
        this.reseultAdd();
      } else {
        this.resultReplace();
    ...
    resultReplace() {
      ...
      if (this.area !== "") {
        ...
        this.nameList.splice(
          ...
          this.getValue(this.area, this.areaList)
        );
      } else if (this.city !== "") {
        ...
        this.nameList.splice(
          0,
          this.nameList.length,
          this.getValue(this.city, this.cityList)
        );
      } else if (this.prov !== "") {
        ...
        this.nameList.splice(
          0,
          this.nameList.length,
          this.getValue(this.prov, this.provList)
        );
      }
    },
    // 删除标签

<template>
  <div class="popup-img-preview" v-if="visible">
    <div class="img-warp" :style="'margin-top:' + (-imgInfo.height * imgInfo.scale) / 2 + 'px;'">
      <img @mousedown.prevent="imgMouseDown" :src="imgSrc" :style="
          'width: ' +
            imgInfo.width * imgInfo.scale +
            'px; left:' +
            imgInfo.left +
            'px; top:' +
            imgInfo.top +
    ...
    <div class="view-btns">
      ...
      <div class="img-infos inline" v-if="imgName === '' || imgName === undefined">
        缩放：{{ Math.round(imgInfo.scale * 100) }}%
      </div>
      <div class="img-infos" v-else>
        {{ imgName }} [ 缩放：{{ Math.round(imgInfo.scale * 100) }}% ]
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    clockwiseRotation() {
      this.imgInfo.rotate =
        this.imgInfo.rotate + 90 === 360 ? 0 : this.imgInfo.rotate + 90;
    },
    // 获取图片尺寸
    getImgSize(url) {

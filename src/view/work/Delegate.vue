  ...
  <el-main>
    ...
    <ContentWarp>
      <el-table border stripe :data="searchInfo.list" size="small">
        ...
        <el-table-column align="center" label="产品名称" prop="productName" width="120" />
        <el-table-column align="center" label="流程名称" prop="processName" min-width="200px" />
        <el-table-column align="center" label="被指派角色" prop="assignedRole.name" width="120" />
        <el-table-column align="center" label="指派方式" prop="type" :formatter="formatValue" width="100" />
        <el-table-column align="center" label="指派角色" prop="roleName" :formatter="formatValue" width="120" />
        ...
        <el-table-column align="center" label="操作" min-width="200px">
          <template slot-scope="scope">
            ...
            <el-button size="mini" :type="scope.row.status === 0 ? 'success' : 'danger'" plain
              @click="openStatus(scope.row)">{{ scope.row.status === 0 ? "启用" : "禁用" }}</el-button>
          </template>
        </el-table-column>
      </el-table>
    </ContentWarp>
    ...
    <el-dialog :title="statusInfo.form.status === '1' ? '启用' : '禁用'" width="600px" center
      ...
      <p class="text-center">
        ...
        }}<span class="text-red"> {{ statusInfo.form.companyName }} </span>-<span class="text-red">
          {{ statusInfo.form.productName }} </span>下<br />
        <template v-if="statusInfo.form.assignedRole !== undefined">
          <span class="text-red">
            {{ statusInfo.form.assignedRole.name }}
          </span>
        </template>的
        <span class="text-red">{{ statusInfo.form.type === "0" ? "人工分配" : "自动派发" }}</span>吗？
      </p>
      <div slot="footer" class="dialog-footer">

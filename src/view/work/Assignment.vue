<template>
  <el-main>
    ...
    <!--禁用或启用窗口-->
    <el-dialog :title="statusInfo.form.status === 0 ? '启用' : '禁用'" width="600px" center
      :visible.sync="statusInfo.isOpen" :show-close="false">
      <p class="text-center">
        确定{{ statusInfo.form.status === 0 ? "启用" : "禁用"
        }}<span class="text-red"> {{ statusInfo.form.companyName }} </span>-<span class="text-red">
          {{ statusInfo.form.productName }} </span>-<span class="text-red"> {{ statusInfo.form.processName }} </span>吗？
      </p>
      <div slot="footer" class="dialog-footer">

	...
	<el-main>
				...
				<el-form-item label="申请金额" prop='loanAmount'>
					<el-select v-model='addInfo.form.loanAmount' placeholder='请选择' clearable>
						<template v-for="(option,index) of amountList">
							<el-option :label='option' :value='index'></el-option>
						</template>
					</el-select>
				</el-form-item>
		...
		<el-dialog title="查看订单" width='400px' center :visible.sync='findInfo.isOpen' :show-close='false'>
			<el-form :model="findInfo.form" label-position='right' label-width="110px" class='find-info-warp'>
				...
				<el-form-item label="申请产品：">{{findInfo.form.productName}}</el-form-item>
				<el-form-item label="申请金额：">{{findInfo.form.loanAmount}}</el-form-item>
				<el-form-item label="借款用途：">{{findInfo.form.used}}</el-form-item>
				<el-form-item label="借款期限：">{{findInfo.form.loanDay + "天"}}</el-form-item>
				<el-form-item label="所在区域：">{{findInfo.form.areaName}}</el-form-item>
				<el-form-item label="收入来源：">{{findInfo.form.income || '未填写'}}</el-form-item>
				<el-form-item label="征信情况：">{{findInfo.form.creditInfo || '未填写'}}</el-form-item>
				<el-form-item label="有无自建房：">{{findInfo.form.building || '未填写'}}</el-form-item>
				<el-form-item label="其它：">{{findInfo.form.other || '未填写'}}</el-form-item>
			</el-form>
			<div slot="footer" class="dialog-footer">

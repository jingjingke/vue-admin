<template>
	<el-col :span="20">
		<div class="company-big-title">公司管理（<span>{{companyData.name}}</span>）</div>
		<SearchWarp>
			<el-form>
				<el-form-item>
					<el-button type="primary" plain @click="$router.push('/group/company/staff')">进入员工管理</el-button>
					<el-button plain @click='openDepartment(companyData.id)'>分配部门</el-button>
					<el-button plain @click='openRole(companyData.id)'>分配角色</el-button>
		...
		<el-table border stripe :data='searchInfo.list' size='small'>
			...
			<el-table-column align='center' label="操作" min-width='550px'>
				<template slot-scope="scope">
					...
					<el-button size="mini" plain @click='openDepartment(scope.row.id)'>分配部门</el-button>
		...
		<el-dialog title="修改公司信息" width='600px' center :visible.sync='changeInfo.isOpen' :show-close='false'>
			<el-form :model="changeInfo.form" ref='formByChange' :rules="companyRules" label-width="120px">
				...
				<template v-if='changeInfo.isOpen === true'>
					...
					<el-form-item label="负责区域" prop='principalArea'>
						<SelectAddress v-model='changeInfo.form.principalArea' :name='changeInfo.form.principalAreaName' multiple btnText='添加'></SelectAddress>
						<div class="disNone">
		...
		<el-dialog title="分配部门" width='360px' center :visible.sync='bangDepartment.isOpen' :show-close='false'>
			<el-table ref='bangDepartment' height="290" :data="bangDepartment.list" border stripe size='small' @selection-change="selectDepartmentChange">
		...
		<el-dialog title="分配部门" width='360px' center :visible.sync='bangRole.isOpen' :show-close='false'>
			<el-table ref='bangRole' height="290" :data="bangRole.list" border stripe size='small' @selection-change="selectRoleChange">
				<el-table-column type="selection" width="60"></el-table-column>

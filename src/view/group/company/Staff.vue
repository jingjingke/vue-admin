<template>
  <el-col :span="20">
    <div class="company-big-title">
      员工管理（<span>{{ companyData.name }}</span>）
    </div>
    <SearchWarp>
      <el-form :inline="true" ref="searchInfo.form" :model="searchInfo.form">
        ...
        <el-form-item>
          <el-button type="success" @click="getListAjax"></el-button>
      ...
      <el-form :model="addInfo.form" ref="formByAdd" :rules="rules" label-width="90px">
        ...
        <el-form-item label="所属角色" prop="roleIdList">
          <el-select v-model="addInfo.form.roleIdList" multiple>
            <template v-for="(option, index) of roleList">
              <el-option :label="option.name" :value="option.id" :key="'roleList' + index"></el-option>
            </template>
          </el-select>
        </el-form-item>
      </el-form>
    ...
    <!--修改密码单独的窗口-->
    <el-dialog :title="'正在修改 ' + passInfo.data.name + ' 的密码'" width="600px" center :visible.sync="passInfo.isOpen"
      :show-close="false">
    ...
    <el-dialog :title="statusInfo.form.status === '1' ? '启用' : '禁用'" width="600px" center
      ...
      <p class="text-center">
        ...
        <span class="text-red">
          {{ statusInfo.form.name }} （{{
            statusInfo.form.loginAccount
          }}） </span>吗？
      </p>
      <div slot="footer" class="dialog-footer">

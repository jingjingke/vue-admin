<template>
	<el-main>
			...
			<el-form :inline="true" ref="searchInfo.form" :model="searchInfo.form">
				...
				<el-form-item label="状态">
					<el-select v-model='searchInfo.form.status' placeholder="请选择" clearable>
						<el-option v-for='option of statusList' :key='option.value' :label='option.name' :value='option.value'></el-option>
					</el-select>
				</el-form-item>
				<el-form-item>
			...
			<el-table border stripe :data='searchInfo.list' size='small'>
				...
				<el-table-column align='center' label="操作" width='180px'>
					<template slot-scope="scope">
						...
						<el-button size="mini" :type="scope.row.status===5?'success':'danger'" plain @click='openStatus(scope.row)'>{{scope.row.status===5?'启用':'禁用'}}</el-button>
					</template>
				</el-table-column>
			</el-table>
		</ContentWarp>
		...
		<!--禁用或启用窗口-->
		<el-dialog :title="statusInfo.form.status===5?'启用':'禁用'" width='600px' center :visible.sync='statusInfo.isOpen' :show-close='false'>
			<p class="text-center">确定{{statusInfo.form.status===5?'启用':'禁用'}}<span class="text-red"> {{statusInfo.form.taskName}} </span>吗？</p>
			<div slot="footer" class="dialog-footer">

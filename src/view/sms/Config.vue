      ...
      <el-form :inline="true" ref="searchInfo.form" :model="searchInfo.form">
        ...
        <el-form-item label="状态">
          <el-select v-model="searchInfo.form.status" placeholder="请选择" clearable>
            <el-option v-for="(option, index) of statusList" :key="index + 1" :label="option" :value="index + 1">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
        ...
        <el-table-column align="center" label="操作" width="140px">
          <template slot-scope="scope">
            ...
            <template v-if="scope.row.status === 3">
              <el-button size="mini" type="danger" plain @click="openReason(scope.row)">查看不过理由
              </el-button>
            </template>
          </template>
        </el-table-column>
